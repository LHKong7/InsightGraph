 ...
 *
 * Cypher statements are reused almost verbatim from the Neo4j writer
 * (packages/graph/src/neo4j/writer.ts) — FalkorDB implements OpenCypher so
 * MERGE / ON CREATE / ON MATCH / relationship pattern creation all behave
 * identically. The main differences are:
 *
 *   1. FalkorDB does not expose multi-statement transactions like Neo4j's
 *      `session.executeWrite()`. Each `graph.query()` call is atomic on its
 *      own (single-threaded server). We accept this — within one document,
 *      partial failures are unlikely and retries are idempotent thanks to
 *      MERGE semantics.
 *
 *   2. Parameters use `{ params: { ... } }` instead of a positional second arg
 *      — centralized in FalkorConnection.query().
 *
 * MERGE semantics for entities follow the same ON CREATE / ON MATCH logic
 * as the Neo4j writer: description preferExisting, aliases = larger set wins.
 */
export class FalkorGraphWriter implements IGraphWriter {
  constructor(private conn: FalkorConnection) {}

  async writeDocument(
    doc: DocumentIR,
    extractions: ExtractionResult,
    // FalkorDB's Cypher MERGE already encodes the default policy behavior.
    // The policy argument is accepted for interface compatibility; custom
    // policies are honored by the SQLite writer today.
    _policy?: MergePolicy,
  ): Promise<Record<string, number>> {
    await this.conn.open();
    return writeAll(this.conn, doc, extractions);
  }
}
