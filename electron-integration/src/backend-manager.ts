...
/**
 * Manages the InsightGraph Node.js backend lifecycle from an Electron main process.
 *
 * Supports two modes:
 * - **source**: runs `node apps/api/dist/index.js` from the project directory
 * - **binary**: spawns a pre-packaged executable
 *
 * @example
 * ```ts
 * const backend = new BackendManager({
 *   mode: "source",
 *   projectDir: path.join(__dirname, "../../InsightGraph/typescript"),
 *   startDocker: true,
...
export class BackendManager extends EventEmitter<BackendManagerEvents> {
  ...
  /** Start Docker (if configured) and the Node.js backend. */
  async start(): Promise<void> {
    // 1. Pre-check environment
    const check = await checkEnvironment({
      needNode: this.cfg.mode === "source",
    ...
    if (this.cfg.mode === "source") {
      this.proc = spawn(
        ...
        { cwd: this.cfg.projectDir, env, stdio: ["ignore", "pipe", "pipe"] },
      );
    } else {
      this.proc = spawn(this.cfg.binaryPath, [], {
        env,
