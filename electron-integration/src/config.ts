...
export interface InsightGraphElectronConfig {
  /** "source" runs from TypeScript source via node; "binary" uses a pre-packaged executable. */
  mode: BackendMode;

  /** Source mode: absolute path to the InsightGraph typescript/ directory. */
  projectDir?: string;

  /** Binary mode: absolute path to the insightgraph-server executable. */
  binaryPath?: string;

  /** API listen host. @default "127.0.0.1" */
  apiHost?: string;
...
export function resolveConfig(
  ...
  return {
    ...
    dockerComposePath: cfg.dockerComposePath ?? "",
